...
- Also open `.env` file in `labs\40-AIAgents`, select and copy all configuration in this file. Then open `.env` file in `apps-rps\rps-game-agent` folder and replace all existing code.

- Verify the game server urls in `.env` file is below.

```
RPS_SERVER_URL=https://aiaaa-s2-game-server.azurewebsites.net/
```
